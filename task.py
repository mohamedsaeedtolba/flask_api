...
def preprocess(data): 
    ...
    for i in range(len(hotels_reviews)):
        if len(str (hotels_reviews[i])) > 500:
...
def main():
    hotels_reviews,data__=preprocess(data)
    #tone_list=tone_analyzer(translate(hotels_reviews)) it return tones it can take hotels_reviews as argument
    #x= input('enter search word or name of hotel  ')
    if str (x) == 'search':
        print(hotels_search(data__))
        #tone_list
    else:
        hotels_names=hotels_search(data__)
        print(hotels_names[str(x)])  # string is name of hotel
if __name__ == '__main__':
    x=str(input('enter search word or name of hotel to get data'))
    main(x)
